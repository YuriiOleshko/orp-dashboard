const title = { id: 'acc.title', defaultMessage: 'Setup Your Account' };
const inputPlaceholderName = { id: 'acc.input-name', defaultMessage: 'First Name' };
const inputPlaceholderLast = { id: 'acc.input-last', defaultMessage: 'Last Name' };
const inputPlaceholderCompany = { id: 'acc.input-company', defaultMessage: 'Name of Organization' };
const inputPlaceholderEmail = { id: 'acc.input-email', defaultMessage: 'E-mail' };
const inputPlaceholderTerms = { id: 'acc.input-terms', defaultMessage: 'By continuing, you agree to OFP Conditions of Use and Privacy Notice.' };
const inputPlaceholderPassword = { id: 'acc.input-pas', defaultMessage: 'Password' };
const inputPlaceholderRePassword = { id: 'acc.input-repas', defaultMessage: 'Repeat Password' };
const btnLabel = { id: 'acc.btn-label', defaultMessage: 'Setup Account' };

export { title, inputPlaceholderName,
  inputPlaceholderLast,
  inputPlaceholderCompany,
  inputPlaceholderEmail,
  inputPlaceholderPassword,
  inputPlaceholderTerms,
  inputPlaceholderRePassword,
  btnLabel };
